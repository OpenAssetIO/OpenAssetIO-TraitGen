...
# Enable CMake's built-in clang-tidy support.
macro(enable_clang_tidy)
    find_program(OPENASSETIO_TRAITGENTEST_CLANGTIDY_EXE NAMES clang-tidy clang-tidy-12)

    if (OPENASSETIO_TRAITGENTEST_CLANGTIDY_EXE)
        # Construct the clang-tidy command line
        set(CMAKE_CXX_CLANG_TIDY ${OPENASSETIO_TRAITGENTEST_CLANGTIDY_EXE})
...
macro(enable_cpplint)
    find_program(OPENASSETIO_TRAITGENTEST_CPPLINT_EXE cpplint)
    if (OPENASSETIO_TRAITGENTEST_CPPLINT_EXE)
        ...
        add_custom_target(
            openassetio-traitgentest-cpplint
            COMMAND ${CMAKE_COMMAND} -E echo "Executing cpplint check..."
            COMMAND
            ${OPENASSETIO_TRAITGENTEST_CPPLINT_EXE}
...
macro(enable_clang_format)
    find_program(OPENASSETIO_TRAITGENTEST_CLANGFORMAT_EXE NAMES clang-format clang-format-12)
    if (OPENASSETIO_TRAITGENTEST_CLANGFORMAT_EXE)
        file(
        ...
        # targets.
        add_custom_target(
            openassetio-traitgentest-clangformat
            COMMAND ${CMAKE_COMMAND} -E echo "Executing clang-format check..."
            COMMAND
            ${OPENASSETIO_TRAITGENTEST_CLANGFORMAT_EXE}
...
macro(enable_cmake_lint)
    find_program(OPENASSETIO_TRAITGENTEST_CMAKELINT_EXE NAMES cmake-lint)
    if (OPENASSETIO_TRAITGENTEST_CMAKELINT_EXE)
        file(
            GLOB_RECURSE _sources
            LIST_DIRECTORIES false
            CONFIGURE_DEPENDS # Ensure we re-scan if files change.
        ...
        # targets.
        add_custom_target(
            openassetio-traitgentest-cmakelint
            COMMAND ${CMAKE_COMMAND} -E echo "Executing cmake-lint check..."
            COMMAND ${OPENASSETIO_TRAITGENTEST_CMAKELINT_EXE} ${_sources}
        )
